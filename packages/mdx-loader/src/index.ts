...
const DEFAULT_OPTIONS: RemarkAndRehypePluginOptions = {
  rehypePlugins: [],
  remarkPlugins: [unwrapMdxCodeBlocks, emoji as any, headings, toc],
...
export default async function mdxLoader(
  ...
  // console.log((reqOptions.remarkPlugins as any)[0][0].toString());
  // remove admonitions from remark plugins
  const optionsRemarkPlugins = (reqOptions.remarkPlugins || []).filter(
    (maybePlugin) => {
      const plugin = Array.isArray(maybePlugin) ? maybePlugin[0] : maybePlugin;
      const isAdmonitions = plugin === admonitions;
      return !isAdmonitions;
    }
  );
  ...
  try {
    ...
    // from https://github.com/microsoft/TypeScript/issues/43329#issuecomment-922544562
    const dynamicImport = Function('return import("@mdx-js/mdx")')() as Promise<
